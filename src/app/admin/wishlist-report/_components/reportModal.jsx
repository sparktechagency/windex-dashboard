...
export default function ReportModal({
  ...
  // Reported profile and wishlist details from user object (mapped from refference in ReportReviewTable)
  const reportedProfile = {
    name: modalData?.user?.name || "N/A",
    email: modalData?.user?.email || "N/A",
    title: modalData?.user?.title || "N/A",
    description: modalData?.user?.description || "N/A",
    link: modalData?.user?.link || "N/A",
    price: modalData?.user?.price || "N/A",
    token: modalData?.user?.token || "N/A",
  };
    ...
    >
      <div className="m-10">
        ...
        <div className="my-5">
          ...
          <RowData name="Reported User Name" value={reportedProfile.name} />
          <RowData name="Reported User Email" value={reportedProfile.email} />
          <RowData name="Wishlist Title" value={reportedProfile.title} />
          <RowData name="Description" value={reportedProfile.description} />
          <RowData name="Link" value={reportedProfile.link} />
          <RowData name="Price" value={reportedProfile.price} />
          <RowData name="Token" value={reportedProfile.token} />
